...
/* Global variables -----------------------------------------------------------*/
static volatile bool button_home_alarm_flag_pressed = false;

/* Function definitions ------------------------------------------------------*/
bool port_button_get_status(uint32_t button_id)
{
    if (button_id == PORT_BUTTON_HOME_ALARM_ID)
    {
        return button_home_alarm_flag_pressed;
    }
    else 
    {
...
void port_button_set_status(uint32_t button_id, bool status)
{
    if (button_id == PORT_BUTTON_HOME_ALARM_ID)
    {
        button_home_alarm_flag_pressed = status;
    }
    else
    {
...
bool port_button_read_value(uint32_t button_id)
{
    if (button_id == PORT_BUTTON_HOME_ALARM_ID)
    {
        ...
        return HAL_GPIO_ReadPin(STM32F4_BUTTON_HOME_ALARM_GPIO_PORT, STM32F4_BUTTON_HOME_ALARM_GPIO_PIN);
    }
    else
    {
...
void port_button_exti_config(uint32_t button_id)
{
    ...
    if (button_id == PORT_BUTTON_HOME_ALARM_ID)
    {
        ...
        HAL_NVIC_SetPriority(STM32F4_BUTTON_HOME_ALARM_EXTI_IRQn, STM32F4_BUTTON_HOME_ALARM_IRQ_PRIO, STM32F4_BUTTON_HOME_ALARM_IRQ_SUBPRIO);
        HAL_NVIC_EnableIRQ(STM32F4_BUTTON_HOME_ALARM_EXTI_IRQn);
    }
}


void port_button_gpio_setup(uint32_t button_id)
{
    ...
    if (button_id == PORT_BUTTON_HOME_ALARM_ID)
    {
        ...
        button.Pin = STM32F4_BUTTON_HOME_ALARM_GPIO_PIN;
        ...
        /* Initialize GPIO pin */
        HAL_GPIO_Init(STM32F4_BUTTON_HOME_ALARM_GPIO_PORT, &button);

        port_button_exti_config(button_id);

        // Ensure the button flag is reset at the beginning
        button_home_alarm_flag_pressed = false;
    }
}