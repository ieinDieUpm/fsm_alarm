...
/* Global variables -----------------------------------------------------------*/
static volatile bool pir_sensor_home_alarm_status = false; /*!< Status of the PIR sensor home alarm */
...
bool port_pir_sensor_get_status(uint32_t pir_sensor_id)
{
    ...
    {
        return pir_sensor_home_alarm_status;
    }
    else
    {
...
void port_pir_sensor_set_status(uint32_t pir_sensor_id, bool status)
{
    if (pir_sensor_id == PORT_PIR_SENSOR_HOME_ALARM_ID)
    {
        pir_sensor_home_alarm_status = status;
    }
    else
    {
...
bool port_pir_sensor_read_value(uint32_t pir_sensor_id)
{
    if (pir_sensor_id == PORT_PIR_SENSOR_HOME_ALARM_ID)
    {
        ...
        return HAL_GPIO_ReadPin(STM32F4_PIR_SENSOR_HOME_ALARM_GPIO_PORT, STM32F4_PIR_SENSOR_HOME_ALARM_GPIO_PIN);
    }
    else
    {
        return false; // Invalid PIR sensor ID
...
void port_pir_sensor_exti_config(uint32_t pir_sensor_id)
{
    ...
    if (pir_sensor_id == PORT_PIR_SENSOR_HOME_ALARM_ID)
    {
        ...
        HAL_NVIC_SetPriority(STM32F4_PIR_SENSOR_HOME_ALARM_EXTI_IRQn, STM32F4_PIR_SENSOR_HOME_ALARM_IRQ_PRIO, STM32F4_PIR_SENSOR_HOME_ALARM_IRQ_SUBPRIO);
        HAL_NVIC_EnableIRQ(STM32F4_PIR_SENSOR_HOME_ALARM_EXTI_IRQn);
    }
}

void port_pir_sensor_gpio_setup(uint32_t pir_sensor_id)
{
    ...
    if (pir_sensor_id == PORT_PIR_SENSOR_HOME_ALARM_ID)
    {
        ...
        /* Initialize GPIO pin */
        HAL_GPIO_Init(STM32F4_PIR_SENSOR_HOME_ALARM_GPIO_PORT, &pir_sensor);

        port_pir_sensor_exti_config(pir_sensor_id);

        // Ensure the PIR sensor is not activated at the beginning
        pir_sensor_home_alarm_status = false;
    }
}